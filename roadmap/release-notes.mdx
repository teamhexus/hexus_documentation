---
title: "Release Notes"
description: "Follow our progress and see what's new in Hexus!"
icon: "notes"
iconType: "solid"
---

## December, 2025

### New Features

**Microphone Recording for Video-to-Flow (🎤)**\
Record audio directly from your microphone when converting videos into flows.

- Better narration control
- No extra recording tools needed

**Real-Time Collaboration with Liveblocks (🤝)**\
Chat and collaborate live inside workflows.

- Faster reviews
- Fewer screenshots and Slack threads

**Advanced HTML Editing with GrapeJS (🧱)**\
A more powerful HTML editor is now available.

- Deeper customization
- Faster load times
- Cleaner editing experience

**Brandkit (🎨)**\
Manage logos, colors, fonts, and brand rules from a dedicated Brandkit section.

- Apply brand consistency everywhere
- Reduce repetitive styling work

**Resizable Workflow Nodes (📐)**\
Resize nodes in the workflow editor for better readability and organization.

**New Workflow Nodes (🧩)**

- Image Node
- Markdown support in Prototype nodes

**Workflow Execution API (⚙️)**\
Frontend is now fully integrated with the workflow execution API.

- Faster runs
- More reliable outputs

### Improvements

**Performance & Stability (⚡)**

- Faster data fetching using SWR
- Optimized DOM snapshot performance
- Reduced AI token usage via smarter HTML processing

**Video & Audio Enhancements (🎬)**

- Higher quality video recording (12Mbps)
- MP4 fallback playback
- Improved TTS pause handling
- Better encoding with Media Bunny

**Workflow Editor Enhancements (🛠️)**

- Sidebar improvements and handle highlighting
- Prototype nodes can be re-run
- Clone nodes now support `htmlUrl`
- Race conditions handled more safely

**General Improvements**

- Cleaner UI with Lucide icons
- Improved responsiveness across start and build pages
- Profile-level settings now applied consistently across video-to-flow records

### Bug Fixes

- Fixed Liveblocks compilation and scaling issues
- Resolved workflow sorting and cloning bugs
- Fixed Chrome Extension polling and modal issues
- Fixed mobile demo player padding and form issues
- Fixed multiple AI, TTS, and prototype edge cases

## November, 2025

### New Features

**Calendly Workflow Controls (📅)**\
Hide or show steps dynamically in product tours when using Calendly.

**Image Uploads for Title Pages & Forms (🖼️)**\
Upload images directly to title pages and forms.

- No external hosting required

**Workflow Image Prompt (🧠)**\
Use image prompts directly inside workflows to guide AI output.

**Chatbox & Workflow Templates (💬)**\
A new chatbox and templates are now available on the workflow page.

- Faster iteration
- Better collaboration

**Demo Player Persistence Improvements (🔁)**\
Improved reliability for saved demo steps and URLs.

**TTS & AI Avatar Enhancements (🎭)**\
Smoother playback and better speaker handling across flows.

**AI Image Editor Expansion (🎨)**\
The AI Image Editor is now available inside How-To guides.

**Showcase Pagination & Filtering (📂)**\
Better control when managing large showcase libraries.

**Upgrade Modals for Advanced Features (⬆️)**\
Clear upgrade prompts for translation and intro/outro features.

### Improvements

**Workflow Editor (🧩)**

- Markdown prompt editor added
- AI messages now visible inside prototype nodes
- Debounced updates for smoother editing
- Improved GrapeJS theme and asset loading

**Onboarding Improvements (🚀)**\
Cleaner onboarding forms and better template guidance.

**General Improvements**

- Cleaner UI for text stories
- Better application of profile-level settings
- Reduced AI token usage across HTML processing
- Improved landing page redirection logic

### Bug Fixes

- Fixed Calendly workflow edge cases
- Resolved duplicate workflow creation issues
- Fixed mobile demo player padding and layout
- Fixed video recording and trimming bugs
- Resolved pricing update and subscription edge cases

## October, 2025

### New Features

**AI Image Editor for How-To Guides (🎨)**\
Generate, edit, and replace images directly inside your How-To guides without leaving Hexus.

- Create visual variations instantly
- Match branding and context automatically
- Perfect for quick updates and persona-specific content

**Demo Player Persistence (🔁)**\
Your demo player step and URL now persist across sessions.

- No more broken links
- Consistent sharing and tracking

**Video Trimmer for Image Containers (✂️)**\
Trim videos directly inside selected image containers.

- Fine-tune visuals without re-uploading
- Faster editing loops

**AI & Language Settings (🌍)**\
Translation Settings are now unified under AI & Language Settings.

- One place to manage languages, AI behavior, and voice settings

**Pause Controls for TTS & AI Avatar (⏸️)**\
Pause and resume Text-to-Speech and AI avatar playback.

- Better pacing
- More control during reviews and demos

**Template Selection During Onboarding (✨)**\
Choose a starting template as part of onboarding.

- Get to value faster
- Skip the blank canvas

**Showcase Folders Improvements (🗂️)**\
Better organization and management of showcases.

**Speaker Precedence for TTS (🎙️)**\
Control which speaker takes priority when generating voiceovers.

**Profile Default Font (📝)**\
Set a default font at the profile level to keep all content consistent.

**Pagination & Filtering on Showcase Page (📑)**\
Navigate large demo libraries with ease.

**Apply to All on CTA Tab (⚡)**\
Apply CTA changes across all relevant steps in one click.

### Improvements

**Video & Audio Enhancements (🎬)**

- Higher quality video recording (12Mbps)
- Improved MP4 encoding for better playback
- Longer, more natural pauses in TTS using break tags

**Workflow & Editor Improvements (🛠️)**

- Complex HTML nodes broken into simpler, editable chunks
- Improved Chrome Extension HTML text replacement API
- Faster build process using Turbo
- Reduced AI token usage via optimized HTML stripping

**General Improvements**

- Faster load times for flow and embed pages using ISR
- Profile-level defaults now apply automatically to video-to-flow records
- Pricing table logic updated for returning paid or trial users

### Bug Fixes

- Fixed TTS duration and avatar playback issues
- Resolved hydration crashes in the demo player
- Fixed guide count race conditions
- Fixed showcase category and filtering issues
- Fixed multiple workflow and CTA edge cases

## September, 2025

**New Features:**

- **HTML Demos (🌍)** - Record your web applications for most realistic demos with Hexus Chrome Extension.
  - Record any web page and adapt it for your viewers with our powerful HTML editor.
  - Replace text and images or hide any section of your web page by simply clicking on the section in our editor.
  - Use **Hexus AI** for bulk editing PII or even editing your images! No downloads, no uploads - just a prompt.
- **AI-Powered Text Editing (✍️)** - You can now highlight any text within your flows and instantly rewrite, shorten, expand, or edit with Hexus AI, making content refinement faster and more intuitive than ever.
  - Perfect for polishing your message on the fly without switching tools
  - Match your brand voice and guidelines
  - Works seamlessly across all text elements in your flows
- **AI Image Editor (🎨)** - Edit and replace images directly on the build page with our new AI-powered image editor, enabling you to generate variations or create entirely new visuals without leaving your workflow.
  - Generate multiple variations from a single image
  - Create custom visuals that perfectly match your content needs and branding
  - Personalize demos for different personas
- **Professional Intros & Outros (🎬)** - Add polished title and end slides to your flows with ease, giving your demos and guides a more structured, presentation-ready appearance. Enable the settings from your Hexus profile and let us handle the rest!
- **Enhanced Rich Text Editor (📝)** - Upload images and videos directly into text blocks with expanded font and styling options for complete creative control over your content presentation.
  - Drag-and-drop media uploads for faster content creation
  - Extended typography controls including custom fonts and sizes
  - Advanced formatting options for better visual hierarchy

**Improvements:**

- **Apply to All Functionality (⚡)** - Save time by applying style changes and AI-driven text replacements across all relevant steps with a single click.
  - Batch update text alignment, font sizes, and colors instantly
  - Apply AI rewrites to multiple similar text elements simultaneously
  - Maintain consistency across your entire flow effortlessly
- **Automatic Speech Transcription (🎤)** - AI Speech steps now automatically transcribe audio content, simplifying voiceover editing and management. Enable this feature in profile settings
- **Showcase Page Enhancements (🖼️)**
  - Improved metadata editor customizing Showcase display tiles
  - Customize settings and theme colors to match your branding
- **Undo/Redo Support (↩️)** - Navigate your Text edits confidently with robust undo and redo functionality in the text editor.
  - Experiment freely knowing you can always revert changes
  - Complete edit history for the current session

## August 2025

**New Features:**

- **Custom AI Writing Guidelines (✨)** - You can now customize AI guidelines in your profile settings for _How-to-Guides, Newsletters, Product Updates, Social Media_ and more, ensuring all generated content matches your preferred writing style perfectly.
- **Universal Image Search & Replace (🔍)** - Search for images across all your Hexus flows directly from the dashboard and replace them at once to keep your flows always up to date with the latest visuals.
- **Marketo Lead Integration (🔗)** - Sync leads data from your Hexus flows directly to your Marketo instance with our newest integration, streamlining your lead management workflow.
- **Google Analytics Integration for Enterprise (📊)** - Enterprise customers can now get Hexus analytics delivered straight to their Google Analytics dashboard for comprehensive performance tracking.
- **One-Click Text Guide Translation (🌍)** - Text Guides can now be instantly translated with a single click under _AI and Language Settings_, making global content distribution easier than ever.
- **German Accent AI Voices (🎙️)** - Expand your reach with new German accent AI voices, perfect for creating localized content for German-speaking audiences.

**Improvements:**

- **Enhanced Smart Captions (💡)** - Hexus-AI has been updated to write improved captions for your recorded flows that focuses on viewer outcomes, making your content more compelling and action-oriented.
- **Advanced AI Voice Search (🎵)** - Search through AI voices easily to find the perfect voice for your flows with enhanced filtering options by language, accent, speaker name, gender, and more.

## **July, 2025**

**Video Recording Mode on Hexus Chrome Extension (📹)**

- Customers on Starter plans and above can now record their screens as video steps directly using the Hexus Chrome Extension.

**Text-Guides: New Workflow for Creating Text Guides (✍️)**

- Launch your stories faster with the new text-based story workflow.
- Choose from multiple formats like blog posts, product launch updates, newsletters, and social media posts.

**Auto Zoom for Flows (🔍)**

- Apply zoom across all steps in your flow with a single click using the new Auto Zoom button in Step Settings.

**Interactive Text Input for Flows (⌨️)**

- Add interactive text inputs to your flows, allowing viewers to type in responses and proceed to the next step.

**Ukrainian and German Language Support (🌍)**

- Translate your flows to Ukrainian and German languages for wider reach, and enhance with our AI voices with improved language support.

## **May, 2025**

Content Editor Management in App (📝)

- Manage multiple HTML replacement rule sets directly in the app.
- Previously limited to the Chrome extension, you can now create, manage, and share multiple configurations natively.

Loom Video Imports to Hexus (📹)

- Import your Loom videos to the **Scratch Page** and seamlessly convert them into Hexus flows.
- Use Loom videos as-is or enhance them with Hexus AI to create interactive flows.

Coming up: Enhanced Video Recording Mode on Chrome Extension (🖥️)

- Record your browser tab, window, or full screen as a single video using the updated Chrome extension.
- Simply click “Record a video” to choose your recording mode and start capturing instantly.

## **April 2025**

Hexus Flow Creation from Scratch (🚀)

- You can now create complete Hexus flows without using the Chrome extension.
- Upload videos, images, or import videos directly from scratch page for a flexible and extension-free setup.

## **March 2025**

AI Avatar Enhancements (🤖)

- Add or replace steps in your demo with AI avatars for a more human, personalized guide experience.
- Perfect for scaling presentations without being on camera.

## February 2025

**New Features:**

**Playlist View for Demos (🪟)**

- Guide prospects through a structured, step-by-step demo experience.
- Seamless navigation ensures a curated demo journey.
- Integrate Hexus interactive demos along with your own content for an all-in-one showcase.

**Enhanced CTAs & Hotspots (👀)**

- Customize width, height, and styling for a tailored experience.
- New directional arrows help guide viewers effortlessly through your demos.

**New Analytics Overview (📊)**

- Get a consolidated view of visitor engagement and conversions in one place.

**Improvements**:

**Undo Functionality (◀️)**

- Instantly revert edits and recover previous versions for better control and flexibility.

**Password Protection for Showcase Pages (🔑)**

- Keep your content secure with enhanced access control.

**AI Studio & Onboarding Enhancements (🎙️)**

- Smoother workflows and improved guidance for a better user experience.

**Expanded Language Support (📝)**

- Now available in two additional languages.

**New Integrations (🧩)**

- Seamlessly export demos to YouTube or embed them in GitBook.

## December 2024

### December 16, 2024

New Features:

1. AI Studio (🤖)
   - Seamlessly share and track your chat history
   - Access our curated Prompts Library
   - Create and save Custom Prompts for your specific needs
   - Set additional context that applies across all prompts for more accurate responses
2. Enhanced Settings Stories  (📖✒️)
   - Use the "/" command to access AI assistance directly within your stories
3. Simplified Demo Creation (🤝)
   - Create comprehensive demos with single video recording in Demo Video Template
   - Reduce editing time and maintain consistent narrative flow

## November 2024

### November 28, 2024

New Features:

1. Early access to AI Studio
   - Explore and ask about anything within the Hexus app or any of your connected apps.
   - Hexus AI is here to assist you with writing summaries, updates, and much more.
2. Improved text-based stories
   - Find more templates available for your stories.
   - You can generate outlines for any template, rearrange them, and create different content types like blogs, newsletters, social media posts, and more. This gives you more flexibility to craft engaging content in no time!

Improvements:

1. Chrome extension updates - You can now adjust your screen size while recording, and use shortcuts to pause or easily capture screenshots.
2. Analytics enhancements - You can now choose the specific date range for viewing your demo analytics. Get more granular insights and track your demo performance more effectively.
3. UI refresh - A more intuitive and visually appealing interface that helps you get to work faster and more efficiently.

### November 8, 2024

New Features:

1. Settings-based Stories (📖✒️)
   - Our text-based stories workflow got a new UI change and features.
   - When you select a text-based story, our AI now generates a set of titles which you can change and rearrange. The rest of the information is filled based on the title. This makes your How-to Guides even more customizable and comprehensive.
2. Embed files into your flow (⚓)
   - You can now embed multiple types of files in your flows using the CTA slide option.
   - If you want your customers to download a PDF, go to a webpage, view a Figma file, or go to your Google drive, you can easily embed these files to your Hexus flow
   - Hubspot users can also tie any leads they get from Hexus demos to their CRM by embedding Hubspot Forms here
3. Embed tracking (⚓📍)
   - With our new Reference Link option in the Embed sharing option, Hexus can now keep a track of all the pages which have the flow embedded in them.
   - Your updated Hexus flows will now automatically refresh and load. No need to add them manually anymore.

## October 2024

### October 25, 2024

New Features:

1. Hexus AI Studio (🤖)
   - Introducing the new Hexus AI Studio. Now you can turn text based prompts into demos, reducing even more time in asset creation for your next GTM campaign!
   - Write your prompt and generate Product demos, Newsletters, Guides and more marketing collateral so you can respond within minutes of a customer asking you for specific demos or guides
   - You can even generate follow up emails to customers or prospects based on the demo you shared with them, improving customer satisfaction and reducing time to value!
2. Improved Collaboration (🤝)
   - Leave feedback directly on your teammate's flows and collaborate in real time with our new comments and reviews features
   - Approve or suggest changes within Hexus itself so your entire team can work on all product collaterals in one platform
3. Applying Themes (🎨)
   - Create consistent, branded themes with preset styles and voiceovers. Apply them from the 3 dot menu in your Edit flow page.
4. Analytics Reporting (📈)
   - If you're into pivot tables and getting granular with analytics, you can now download your Hexus analytics in CSV format for deeper insights.

### October 4, 2024

New Features:

1. Auto Merge Steps (🔀)
   - Our new auto-merge feature combines similar steps into one.
   - Eliminate deleting redundant steps and publish flows faster
2. Watermark updates (💧)
   - Leverage your watermark as a branding and engagement opportunity
   - Add your watermark with a URL and custom Alt text from Settings \> Customization
3. UI updates (🎨)
   - Add steps without needing to reorder them
   - Edit hotspots, annotate images, blur images from the top toolbar
   - You can now uplevel your demo analytics game with Hexus. In the new analytics UI, you can now:
     - Select custom date ranges
     - View analytics for multi-flow and showcase pages
     - Explore new visualizations and data types including country and referrer info

## September 2024

### September 27, 2024

Improvements:

1. Multi-select Flows ✅: Bulk actions made easy! Select multiple flows to move them to workspaces, folders, or delete them in one go.
2. Image State Management 🖼️: Your edits are safe with us! Reopen the image editor anytime to pick up right where you left off.

New Features:

1. New Cursor Styles (🖱️)
   - Introducing three new cursor styles and various other customizations!
   - Based on your brand guidelines and preferences, you can choose from a hotspot, pointer, or hand cursor
   - You can also choose from a cursor that is filled with color or just an outline
   - All of our cursors blink and you can now adjust the speed at which they do so
   - You can also change the size of all the cursors. This means, based on the size of the element you're trying to point at, the cursor is now customizable!
2. SDK for Showcase Pages (🔗)
   - Embed your awesome showcase pages directly into your help center or website!
3. Folders (📁)
   - Experience improved organization and collaboration with our new folder feature
   - Create folders in your dashboard to keep everything tidy and easy to find. For enterprise users only.

## June 2024

### June 12, 2024

Improvements:

1. Improved export quality for videos

### June 7, 2024

New Features:

1. AI-Powered Personalized Showcase Pages (✨)
   - Introducing our latest feature: AI-powered personalized showcase pages.
   - Host your product tours and guides on a showcase that dynamically personalizes titles, captions, and format based on each unique customer segment.
   - Our AI-powered search feature allows customers to ask questions about your product, and the most relevant video or step-by-step guide will be generated on the fly.

Bug fixes:

1. Fixed a UI bug where users couldn’t access a new flow from the dashboard

## May 2024

### May 14, 2024

Bug fixes:

1. Fixed an issue where some users were having issues with form submissions on embedded iframes.

### May 8, 2024

New Features:

1. Enhanced Chrome Extension (🖥️)
   - Our Chrome extension has been updated with new capabilities.
   - Capture not only static screens but also screen interactions such as scrolling and typing.
   - Watch an example to see the enhanced extension in action.
2. Branched Tours (🌿)
   - Create an index of tours and link them within the same iframe.
   - Guide users through different paths based on their preferences.
   - View an example tour to see how branched tours work.
3. Record Through Step (🖥️🎥)
   - Record your entire desktop screen or camera and add it as a step in your tours.
   - Capture comprehensive and informative content to enhance your tours.
4. Advanced Analytics (📊)
   - View engagement data for each step and track performance over time.
   - Contact us to enable these powerful analytics features and gain deeper insights.
5. More Customization (🎨)
   - Further customize your tours by changing the color of the browser frame.
   - Call out specific focus areas of the screen to draw attention to key elements.
   - Make your tours truly unique and engaging.
6. Optimized for Mobile (📱)
   - The Hexus experience has been optimized for mobile devices.
   - Your tours will look and work great on any screen size.

## April 2024

### April 22, 2024

New Features:

1. Wistia Integration (🚀)
   - Seamlessly publish your product tours and demo videos to Wistia.
   - Add your Wistia API key in the Integrations tab to enable this feature.
   - Learn more about the integration process in our documentation.
2. Background Music (🎶)
   - Enhance the user experience by adding background music to your product tours and demo videos.
   - Create a more engaging and immersive environment for your audience.
   - Choose from a variety of music options to match your content and brand.
3. Tags (🏷️)
   - Organize your workspace by adding tags to your flows.
   - Keep everything well-structured and easily accessible.
   - Find your flows faster than ever with the improved organization system.
4. Custom Background (🖼️)
   - Upload a custom background that can be added to your steps or forms.
   - Personalize your product tours and demo videos to align with your brand.
   - Contact us to learn more about plans that enable custom assets.

Improvements:

- Export as PDF and HTML (📄): Your exported content is now available in PDF and HTML formats, providing more versatility and flexibility.

Bug Fixes:

- Hotspot UI Issue (🎯): We have resolved an issue with the hotspot user interface to ensure a smooth and intuitive experience.

### April 15, 2024

New Features:

1. Custom Branding (🎨)
   - Customize your product tours to align with your brand guidelines.
   - Incorporate custom fonts, watermarks, and other assets to make your tours truly your own.
   - Contact us to learn about plans that enable this feature.
2. Closed Captions (📝)
   - Generate transcripts for your videos with just one click.
   - Make your content accessible to a wider audience, including those with hearing impairments.
   - Contact us to learn about plans that enable this feature.
3. New Language Support: Brazilian Portuguese (🌎)
   - We've expanded our language support to include Brazilian Portuguese (PT-BR).
   - Reach even more users around the world and cater to a broader audience.
4. New Cursor Type (🖱️)
   - We've introduced a new cursor type to provide more options for creating engaging product tours.
   - Enhance the interactivity and visual appeal of your tours with this new addition.

Improvements:

- Auto-refresh (Beta) (🔄): Save time and effort with our auto-refresh feature. It automatically recaptures a video when any UI changes are detected.
- Settings-to-speech (🗣️): Explore a variety of 40 AI voices to find the perfect narrator for your tours. Preview different styles to match your content.

### April 5, 2024

New Features:

1. Stale Content Notifications (📅)
   - Set email reminders to review your Hexus flows and keep them up to date.
   - Never let your content go stale again!
2. Drafts (📝)
   - Any edits made to your flows are now automatically saved as drafts.
   - Work on your flows without worrying about losing progress.
   - Simply hit the publish button to sync your changes with the live share link.
3. Flow Search (🔍)
   - Find your flows quickly and easily with our enhanced search functionality.
   - Search by title, captions, element names, or even URLs of screens you have recorded.
   - Looking for a flow with a specific CTA? Just search for it!
4. Hide Hotspot (🙈)
   - Let your screens speak for themselves by hiding hotspots in individual steps or for all steps at once.
5. Background Music (🎶)
   - Elevate your demo videos with our selection of background music options.
   - Choose the perfect track to complement your content.

Improvements:

- GIF & Video Export (🎥): The quality of your exported content has been further enhanced for an even better user experience.
- Form Leads (📈): Find your form leads conveniently located under the analytics tab for easy access and analysis.

## March 2024

### March 29, 2024

**New Features:**

1. Upload Media (📹)
   - Users can now upload their own MP4, MOV, JPG, PNG, GIF, and WEBM files (up to 5MB) as steps in product tours.
2. Enhanced Step Settings (🕰️)
   - Configure the duration of each step and set how long it should play before transitioning to the next one.
   - Enable autoplay to loop product tours continuously.
3. Improved Settings-to-Speech (🎙️)
   - Adding voiceovers to steps is now more intuitive and user-friendly with a single click.
4. Duplicate Flows (📋)
   - Easily duplicate flows and create versions for efficient tour management.
5. Aspect Ratio Modification (📏)
   - Modify the aspect ratio for all screens within a product tour to optimize for different devices.

**Enhancements:**

- Improved user interface for a smoother tour creation experience.
- Optimized performance for faster loading times.

**Bug Fixes:**

- Resolved an issue where certain file types were not uploading correctly.
- Fixed a bug that caused inconsistent step durations in some cases.